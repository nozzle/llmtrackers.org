    ...
    <section className={className ?? "mb-12"}>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {company.plans.map((plan) => (
          <PlanCard key={plan.slug} plan={plan} onEdit={onEditPlan} />
        ))}
      </div>
    </section>
  );
...
function PlanCard({ plan, onEdit }: { plan: Plan; onEdit: (plan: Plan) => void }) {
  return (
    <div className="group relative rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      <button
        type="button"
        onClick={() => {
          onEdit(plan);
      ...
      <div className="mb-4 flex items-start justify-between">
        ...
        <div className="text-right">
          <div className="text-2xl font-bold text-gray-900">
            {plan.price.amount !== null ? `$${plan.price.amount.toLocaleString()}` : "Custom"}
          </div>
          <div className="text-xs text-gray-500">
            /{plan.price.period}
            {plan.price.note && <span className="block text-gray-400">{plan.price.note}</span>}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 text-sm">
        <div className="flex justify-between">
          ...
          <span className="font-medium">{plan.aiResponsesMonthly?.toLocaleString() ?? "-"}</span>
        </div>
        <div className="flex justify-between">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-medium">
            {plan.locationSupport === "global" ? "Global" : `${plan.locationSupport} regions`}
          </span>
        </div>
        <div className="flex justify-between">
          ...
          {LLM_KEYS.map((key) => (
            <span
              ...
              className={`rounded px-2 py-0.5 text-xs ${
                plan.llmSupport[key] ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-400"
              }`}
            >
        ...
        <div className="mt-4 border-t border-gray-100 pt-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {plan.integrations.map((int) => (
              <span key={int} className="rounded bg-blue-50 px-2 py-0.5 text-xs text-blue-700">
                {int}
              </span>
            ))}
          </div>
...
export function ScreenshotsSection({
  ...
  if (company.screenshots.length === 0) return null;
  ...
  return (
    <section className={className ?? "mb-12"}>
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-gray-600">
          Curated first-party product imagery collected from {company.name}&rsquo;s public site and
          help resources.
        </p>
      </div>
      ...
      <div className="-mx-1 flex gap-3 overflow-x-auto px-1 pb-3">
        {company.screenshots.map((screenshot, i) => (
          <button
            ...
            type="button"
            onClick={() => {
              onOpenMedia({ type: "screenshot", index: i });
            }}
...
export function VideosSection({
  ...
  if (company.videos.length === 0) return null;
  ...
  return (
    <section className={className ?? "mb-12"}>
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-gray-600">
          Curated product walkthroughs and demos for {company.name}, playable without leaving the
          page.
        </p>
      </div>
      ...
      <div className="-mx-1 flex gap-3 overflow-x-auto px-1 pb-3">
        {company.videos.map((video, i) => (
          <button
            ...
            type="button"
            onClick={() => {
              onOpenMedia({ type: "video", index: i });
            }}
            className="group flex-none"
          >
...
export function RatingsSection({ company, className }: { company: Company; className?: string }) {
  const hasAny = REVIEW_SITE_PLATFORMS.some((platform) => company.reviewSites[platform]);
    ...
    <section className={className ?? "mb-12"}>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {REVIEW_SITE_PLATFORMS.map((platform) => {
          ...
          if (!site) return null;
          return <ReviewSiteCard key={platform} platform={platform} site={site} />;
        })}
      </div>
    </section>
  );
...
function ReviewSiteCard({
  ...
  site: NonNullable<Company["reviewSites"][ReviewSitePlatform]>;
}) {
  return (
    <section
      ...
      {site.ratingDistribution.length > 0 && (
        <div className="mt-5 space-y-2">
          ...
          {site.ratingDistribution.map((bucket) => {
            const maxCount = Math.max(...site.ratingDistribution.map((entry) => entry.count), 1);
            ...
            return (
              <div key={`${platform}-${bucket.label}`} className="flex items-center gap-3">
                ...
                <div className="w-12 text-right text-sm text-gray-500">{bucket.count}</div>
              </div>
            );
          })}
        </div>
          ...
          {site.reviews.map((review, index) => (
            ...
            >
              <div className="mb-2 flex flex-wrap items-center gap-x-2 gap-y-1">
                ...
                {review.title && <span className="text-sm text-gray-600">{review.title}</span>}
                {review.rating != null && (
...
export function PublishedReviewsSection({
  company,
  relatedReviews,
  className,
}: {
...
export function TweetsSection({ company, className }: { company: Company; className?: string }) {
  if (company.tweets.length === 0) return null;
...
export function LinksSection({ company, className }: { company: Company; className?: string }) {
  return (
    <section className={className ?? "flex flex-col gap-3 sm:flex-row sm:gap-4"}>
      {company.pricingUrl && (
        <a
...
// ---------------------------------------------------------------------------
// Header Section (shared across all layouts)
// ---------------------------------------------------------------------------
